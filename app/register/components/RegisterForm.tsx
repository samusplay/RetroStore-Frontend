...
export function RegisterForm() {
  ...
  const onSubmit: SubmitHandler<RegisterInput> = async (data) => {
    ...
    try {
      ...
      const msg: string = err.message || 'Error de comunicación';

      // Detectamos si es un error de duplicado
      const isDuplicate =
        msg.toLowerCase().includes('usuario') ||
        msg.toLowerCase().includes('email') ||
        msg.toLowerCase().includes('existe') ||
        msg.toLowerCase().includes('use') ||       // "already in use"
        msg.toLowerCase().includes('registrado');

      if (isDuplicate) {
        await Swal.fire({
          title: '¡BOWSER BLOQUEA EL PASO!',
          text: msg,
          imageUrl: '/images/bowser.gif',
          imageWidth: 120,
          imageHeight: 120,
          imageAlt: 'Bowser bloqueando',
          width: '24em',
          padding: '1.5em',
          background: '#0a0514',
          color: '#f87171',
          ...
          customClass: {
            popup: 'border-2 border-red-600 shadow-[0_0_40px_rgba(220,38,38,0.5)] rounded-2xl',
            title: 'font-mono text-base tracking-[0.12em] mt-2 text-red-400',
            htmlContainer: 'font-mono text-xs text-zinc-300 tracking-wider',
            confirmButton: 'font-mono text-xs tracking-widest uppercase',
          },
        });
      } else {
        // Error genérico — toast chiquito como antes
        toast.error(msg);
      }
    }
  };
          ...
          <div className="grid grid-cols-4 gap-2">
            {CONSOLES.map((c) => {
              ...
              return (
                <label
                  key={c.id}
                  className={`flex flex-col items-center justify-center gap-1.5 rounded-lg border p-2 cursor-pointer transition-all duration-300 ${isSelected
