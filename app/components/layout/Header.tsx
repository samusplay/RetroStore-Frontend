...
export default function Header() {
  ...
  const isCollector = hydrated && user?.role === "COLLECTOR";
      ...
      <header className="sticky top-0 z-50 bg-[#0a0a0f]/80 backdrop-blur-md border-b border-cyan-500/30 shadow-[0_4px_30px_rgba(6,182,212,0.15)]">
        <div className="container mx-auto px-6 h-20 flex items-center justify-between">
          ...
          <nav className="hidden md:flex gap-10 font-medium text-sm text-gray-400">
            <Link href="/" className={`transition-all hover:text-cyan-400 hover:drop-shadow-[0_0_8px_rgba(34,211,238,0.8)] ${pathname === "/" ? "text-cyan-400" : ""}`}>
              Inicio
            </Link>
            <Link href="/catalogo" className={`transition-all hover:text-fuchsia-400 hover:drop-shadow-[0_0_8px_rgba(217,70,239,0.8)] ${pathname.includes("/catalogo") && !pathname.includes("retro-tv") ? "text-fuchsia-400" : ""}`}>
              Catálogo
            </Link>
            ...
            {isSeller && (
              <>
                ...
                <Link href="/perfil/dashboard" className={`transition-all hover:text-cyan-400 hover:drop-shadow-[0_0_8px_rgba(34,211,238,0.8)] font-bold flex items-center gap-2 ${pathname === "/perfil/dashboard" ? "text-cyan-400 drop-shadow-[0_0_8px_rgba(34,211,238,0.8)]" : ""}`}>
                  <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            ...
            <Link href="/catalogo/retro-tv" className={`hover:text-purple-400 hover:drop-shadow-[0_0_8px_rgba(168,85,247,0.8)] transition-all flex items-center gap-2 ${pathname.includes("retro-tv") ? "text-purple-400" : ""}`}>
          ...
          <div className="flex items-center gap-4 min-w-35 justify-end">
            {!hydrated ? (
              ...
              <div className="flex items-center gap-4">
                ...
                {/* Carrito — visible para todos los usuarios autenticados */}
                <>
                  <div className="w-px h-8 bg-white/10" />
                  <Link
                    href="/payments"
                    className="relative flex items-center gap-2 px-4 py-2 rounded-full border border-cyan-500/50 text-cyan-400 hover:bg-cyan-500/10 hover:shadow-[0_0_15px_rgba(0,245,255,0.3)] text-sm font-bold transition-all duration-300"
                    title="Carrito"
                  >
                    ...
                    {/* Badge con contador */}
                    {cartCount > 0 && (
                      <span className="absolute -top-1.5 -right-1.5 w-5 h-5 rounded-full bg-fuchsia-500 text-white text-[10px] font-black flex items-center justify-center shadow-[0_0_8px_rgba(217,70,239,0.8)]">
                        {cartCount}
                      </span>
                    )}
                  </Link>
                ...
                {/* Mis Pagos — solo collectors */}
                {isCollector && (
                  <>
                    <div className="w-px h-8 bg-white/10" />
                    <Link
                      href="/payments/my-payments"
                      className={`flex items-center gap-2 px-4 py-2 rounded-full border border-cyan-500/50 text-cyan-400 hover:bg-cyan-500/10 hover:shadow-[0_0_15px_rgba(0,245,255,0.3)] text-sm font-bold transition-all duration-300 ${pathname === '/payments/my-payments' ? 'bg-cyan-500/10' : ''}`}
                      title="Mis Pagos"
                    >
                      <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
